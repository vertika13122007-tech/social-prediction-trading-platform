...
import {
  ...
  Lock, CheckCircle, XCircle, Search, Calendar, Filter,
  ShoppingBag, Award, RefreshCw, Gift, AlertCircle,
  ArrowUpRight, ArrowDownLeft, Wallet, X, ChevronDown,
  BarChart3, Star, Flame, Activity
} from "lucide-react";

// ─── DATA ────────────────────────────────────────────────────────────────────
const OPEN_POSITIONS = [
  { id: 1, title: "Bitcoin to break $150K in 2026",  category: "Trends",   prediction: "YES", buyPrice: 0.68, currentPrice: 0.74, qty: 150, totalInvest: 10200, status: "LIVE",   timeLeft: "2d 12h", investors: 312, winProb: 72, roi: "+8.8%", pnl: +900  },
  { id: 2, title: "Lakers to win NBA Championship 2027", category: "Sports",  prediction: "YES", buyPrice: 0.55, currentPrice: 0.48, qty: 200, totalInvest: 11000, status: "LIVE",   timeLeft: "11d 3h", investors: 187, winProb: 48, roi: "-12.7%",pnl: -1400 },
  { id: 3, title: "Tesla stock to reach $500 in Q3 2026",category: "Products",prediction: "NO",  buyPrice: 0.42, currentPrice: 0.52, qty: 100, totalInvest: 4200,  status: "LIVE",   timeLeft: "1d 8h",  investors: 289, winProb: 55, roi: "+23.8%",pnl: +1000 },
  { id: 4, title: "Mr Beast to hit 500M subscribers", category: "Creators", prediction: "YES", buyPrice: 0.70, currentPrice: 0.70, qty: 80,  totalInvest: 5600,  status: "CLOSED", timeLeft: "Ended",  investors: 98,  winProb: 65, roi: "0%",   pnl: 0     },
];

const MARKETPLACE = [
  { id: 1, seller: "CryptoKing",   market: "Bitcoin to break $150K",   prediction: "YES", sellPrice: 0.76, roi: "+11.7%", qty: 50,  listed: "1h ago"  },
  { id: 2, seller: "TechOracle",   market: "AI replace developers 2027",prediction: "YES", sellPrice: 0.62, roi: "+3.3%",  qty: 120, listed: "3h ago"  },
  { id: 3, seller: "WallStreetPro",market: "Tesla stock to $500",       prediction: "NO",  sellPrice: 0.48, roi: "+14.3%",qty: 30,  listed: "30m ago" },
  { id: 4, seller: "MarketGuru",   market: "Ronaldo 50+ goals season",  prediction: "YES", sellPrice: 0.58, roi: "+5.4%",  qty: 90,  listed: "5h ago"  },
];

const HISTORY = [
  { id:1,  type:"Won",     market:"Bitcoin to $100K",        date:"22 Jun 2026, 3:45 PM", txId:"TXN001", invested:5000, returned:9250,  pnl:+4250, status:"Completed" },
  { id:2,  type:"Bought",  market:"Lakers NBA Championship",  date:"20 Jun 2026, 1:20 PM", txId:"TXN002", invested:11000,returned:0,     pnl:0,     status:"Active"    },
  { id:3,  type:"Sold",    market:"Tesla $500 Q3",            date:"19 Jun 2026, 6:00 PM", txId:"TXN003", invested:4200, returned:5200,  pnl:+1000, status:"Completed" },
  { id:4,  type:"Lost",    market:"Dogecoin to $1",           date:"15 Jun 2026, 9:30 AM", txId:"TXN004", invested:3000, returned:0,     pnl:-3000, status:"Completed" },
  { id:5,  type:"Reward",  market:"Season 2 Winner Badge",    date:"14 Jun 2026, 12:00 PM",txId:"TXN005", invested:0,    returned:1000,  pnl:+1000, status:"Completed" },
  { id:6,  type:"Bought",  market:"Mr Beast 500M Subscribers",date:"10 Jun 2026, 2:15 PM", txId:"TXN006", invested:5600, returned:0,     pnl:0,     status:"Active"    },
  { id:7,  type:"Partial", market:"Tesla $500 Q3 (25%)",      date:"9 Jun 2026, 11:00 AM", txId:"TXN007", invested:1050, returned:1300,  pnl:+250,  status:"Completed" },
  { id:8,  type:"Refund",  market:"Lakers Championship",      date:"5 Jun 2026, 8:00 AM",  txId:"TXN008", invested:2000, returned:2000,  pnl:0,     status:"Completed" },
];
...
// ─── SELL MODAL ───────────────────────────────────────────────────────────────
function SellModal({ position, onClose }) {
        ...
        <AnimatePresence mode="wait">
          {success ? (
            ...
            <motion.div key="form" className="px-6 py-6 space-y-5">
              ...
              <div className="flex gap-3">
                <button onClick={onClose} className="flex-1 py-3 rounded-2xl border-2 border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 font-semibold text-sm hover:border-gray-300 active:scale-95 transition-all">Cancel</button>
                <motion.button onClick={()=>{setLoading(true);setTimeout(()=>{setLoading(false);setSuccess(true)},1200)}} disabled={loading}
...
export default function Portfolio() {
  ...
  const totalInvest  = OPEN_POSITIONS.reduce((s,p)=>s+p.totalInvest, 0);
  const totalPnl     = OPEN_POSITIONS.reduce((s,p)=>s+p.pnl, 0);
  const totalReturn  = totalInvest + totalPnl;
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-gray-950 transition-colors duration-200">
        ...
        <main className="flex-1 min-w-0 px-4 py-6 space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              { label:"Positions",      value:`${OPEN_POSITIONS.length} Active`,  icon:<Activity size={18}/>,   bg:"bg-purple-100 dark:bg-purple-900/30",ic:"text-purple-600 dark:text-purple-400" },
            ].map((s,i)=>(
      ...
      {/* Modals */}
      <AnimatePresence>
        {sellPosition && <SellModal position={sellPosition} onClose={()=>setSellPosition(null)}/>}
